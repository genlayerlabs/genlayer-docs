...
The `get_webpage_with_principle` is an abstraction from the `EquivalencePrinciple` function, used to retrieve data from webpages. Validators will perform similar tasks and compare their results with the leader's result to ensure that the data fetched from the web source meets the condition defined in the Equivalence Principle.

#### Format of the webpage data
You can get the webpage data as text or as HTML by specifying the format parameter `format="text"` or `format="html"` as follows:

##### Example 1: Getting the webpage data as text
```python copy
...
##### Example 2: Getting the webpage data as HTML
```python copy
weather_data = await get_webpage_with_principle(
    self.weather_url,
    eq_principle="The data must be up-to-date and accurately reflect current weather conditions",        
...
<Callout type="info">
    Note that the get_webpage_with_principle doesn't require the comparative option. All the validators will request the same data from the web source.
</Callout>
