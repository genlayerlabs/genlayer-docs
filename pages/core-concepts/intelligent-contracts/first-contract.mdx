...
### Magic comment
First of all, you need to place a magic comment with version of GenVM you wish to use on the **first** line of your file
```py
...
- All persistent data should be declared as class fields and annotated with types
    ...
    - `int` type isn't supported on purpose. Most likely you wish to use some fixed-size integer type, such as `i32` or `u256`. If it is not the case and you are sure that you need big integers you can annotate your field with `bigint`, which is just an alias for python `int`
...
<Callout emoji="👾">
  The GenLayer simulator automatically detects the constructor parameters from your code. When you run your contract in the simulator, it provides the UI for all parameters for both constructor and methods, making it easy to manage and modify these inputs.
</Callout>
