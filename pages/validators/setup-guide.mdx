...
2. Download the packaged application
   ```sh copy
   export version=v0.5.1 # set your desired version here
...
| `consensus.consensusaddress` | `GENLAYERNODE_CONSENSUS_CONSENSUSADDRESS` |
...
```sh copy
# Override the consensus address
export GENLAYERNODE_CONSENSUS_CONSENSUSADDRESS="0x..."
...
<Callout type="info">
  Note: If you are using the default configuration, `genvm.start_manager` is
...
source .env && docker run --rm --env-file ./.env \
  ...
  yeagerai/genlayer-node:${NODE_VERSION:-v0.5.1} \
  ./bin/genlayernode doctor
```
