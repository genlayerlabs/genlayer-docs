...
##### Why Use a dedicated Operator Address?

<Callout type="info">
    **Recommended:** Use a separate operator address for security.
</Callout>

- **Owner address** - Holds your staked funds (keep offline/cold)
- **Operator address** - Signs blocks and performs validator duties (hot wallet on server)

This way, if your operator server is compromised, your staked funds remain safe. This also enables delegating the validator operation to an external staking provider.

You can change the operator at any time:
