...
rollup:
  genlayerchainrpcurl: "https://asimov-chain-1.genlayerlabs.com/rpc" # GenLayer Chain RPC URL
  genlayerchainwebsocketurl: "wss://asimov-chain-1.genlayerlabs.com/ws" # GenLayer Chain WebSocket URL
...
or simply use the provided `docker-compose.yaml` and the `.env.example` from the downloaded application packaged.
...
source .env && docker run --rm --env-file ./.env \
  ...
  yeagerai/genlayer-node:${NODE_VERSION:-latest} \
  ./bin/genlayernode doctor
```
...
You can forward your logs and metrics to external systems for centralized monitoring and alerting by using the service `alloy` provided in the `docker-compose.yaml` from the downloaded application packaged.
