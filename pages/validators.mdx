...
<Callout emoji="⚠️">
  Currently the only supported LLM provider is OpenAI. More providers will be
...
1. Download the packaged application
   - Go to https://github.com/yeagerai/genlayer-node/releases and download the corresponding asset. For example `genlayer-node-linux-amd64-v0.1.0.tar.gz`
   - Or download with `wget`
     ```sh
     wget https://github.com/yeagerai/genlayer-node/releases/download/v0.1.0/genlayer-node-linux-amd64-v0.1.0.tar.gz
     ```
1. Extract it
   ```sh
   tar -xzvf genlayer-node-linux-amd64-*.tar.gz
   ```
1. `cd` into it
   ```sh
...
1. Run the node
   ```sh
   OPENAIKEY='sk-proj-fill-in-your-openai-key' ./bin/genlayernode
   ```
...
rollup:
  ...
  ethclienturl: "https://rpc.ankr.com/eth_sepolia" # Ethereum RPC URL
  ethclientwebsocketurl: "wss://sepolia.infura.io/ws/v3/" # Ethereum WebSocket URL
consensus:
  ...
  contractghostaddress: "0x..." # GhostManager Smart Contract Address
