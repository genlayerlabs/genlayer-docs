...
1. If necessary, update configurations in `./configs/genvm*.yaml`; all of them have a json schema attached. Default ones should work fine out of the box. There are following configurations:
   - `genvm-module-llm.yaml` – configuration of llm module. You may wish to check environment variable names for LLM API keys or put them into config as plain text
   - `genvm-module-web.yaml` – configuration of webdriver module
   - `genvm.yaml` – "genvm" configuration
2. Run precompilation of pre-delivered wasm modules (this step is optional but is **highly** recommended)
   ```bash
   ./bin/genvm precompile
   ```
   Precompilation should be ran after switching to a different node release
...
#### Greyboxing LLMs
`genvm-module-llm` provides user with ability to customize [greyboxing](/_temp/security-and-best-practices/grey-boxing) via lua scripting. Right now users can customize prompt templates, specify temperature and system prompt.
...
## Running the node
   ...
   2. Run two services (modules) in background (this is a crucial step for running *Intelligent* contracts). This can be done automatically or manually.
      - To start them automatically in node configuration set `genvm.manage_modules` to `true`
      - To start them manually run
        ```bash
        ./bin/genvm-module-web &
        HEURISTKEY="<your-key>" ./bin/genvm-module-llm run &
        ```

      More providers are available in the llm module config

   They will be started automatically by the node if configuration item  is set to `true`

   3. Run the node

      ```sh
      export LLM_PROVIDER_KEY='your_llm_provider_api_key' # Set the env var if `genvm.manage_modules` is `true`
