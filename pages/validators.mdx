...
1. If necessary, update configurations in `./configs/genvm*.yaml`; all of them have a json schema attached. Default ones should work fine out of the box
2. Run precompilation of pre-delivered wasm modules (this step is optional but is **highly** recommended)
   ```bash
   ./bin/genvm precompile
   ```
3. Run two services (modules) in background (this is a crucial step for running *Intelligent* contracts):
   1. `./bin/genvm-module-web`
      This module in turn requires a running webdriver, which is packaged in a docker
   2. `./bin/genvm-module-llm run`
      This module requires API keys. See config for more details, or just `export HEURISTKEY=<your-key>` prior to running the module. More providers are available in the config
...
#### Greyboxing LLMs
`genvm-module-llm` provides user with ability to customize greyboxing via lua scripting. Right now users can customize prompt templates, specify temperature and system prompt.
