...
1. Go to https://github.com/yeagerai/genlayer-node/releases and download the corresponding asset. For example `genlayer-node-linux-amd64-v0.1.0.tar.gz`
1. Extract it
   ```sh
...
1. `cd` into it
   ...
   ```
1. [Configure `configs/node/config.yaml`](#config-yaml)
1. [Configure `configs/node/wallet.yaml`](#wallet-yaml)
1. Run the node
   ```sh
   ./bin/genlayernode
   ```
