...
The GenLayer Optimistic Democracy consensus mechanism then selects a leader validator node from among the GenLayer validator nodes to process the current transaction. The Rollup (Sequencer) `Consensus.sol` smart implements the GenLayer protocol’s consensus mechanism. This step is pivotal in maintaining the integrity and trustworthiness of the GenLayer network. The Consensus mechanism also determines the gas costs associated with each transaction.
