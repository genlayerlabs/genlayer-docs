...
const transactionHash = await client.writeContract({
  ...
  leader_only: false, // Optional: whether the transaction should be executed only by the leader
});
```
...
- `leaderOnly`: (Optional) Whether the transaction should be executed only by the leader. Note: this is only supported in the local network.
...
**Returns:** A ContractSchema object describing the contract's interface
<br/>
<hr/>
### getContractSchemaForCode

Retrieves the schema for undeployed contract code.
```typescript
const schema = await client.getContractSchemaForCode(contractCode);
```

**Parameters:**
- `contractCode`: The contract source code as a string
