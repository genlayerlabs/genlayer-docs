...
- `request` (object, required): The contract call request parameters
  ...
  - `status` (string, optional): The transaction status filter (`"finalized"` or `"accepted"`). If not provided, uses finalized state
  - `value` (string, optional): The value to send with the call (hex-encoded)

**Returns:** Hex-encoded result data

**Example - Read Call (Latest Finalized State):**
...
- Valid `status` values are `"finalized"` (default) and `"accepted"`
- When both parameters are omitted, the call executes against the latest finalized state
...
**Example Request - Latest Finalized State:**
