...
4. **Withdrawing Stake**: After the finality period, the validator and their delegators can withdraw their stake. The stake, including any rewards, is then returned to the appropriate accounts.

## Purpose of Unstaking

The unstaking process serves several important purposes:

1. **Ensuring Accountability**: The finality period ensures that any outstanding violations or potential appeals are resolved before a validator's stake is returned. This prevents validators from leaving the network without addressing potential issues.
...
For validators, the unstaking process means they must plan ahead if they wish to stop validating, as the finality period can vary. For delegators, unstaking their tokens requires them to wait for the validator to go through the unstaking process, which protects their interests by ensuring any penalties or rewards are accounted for before withdrawal.
