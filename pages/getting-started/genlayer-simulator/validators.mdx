...
In the GenLayer Simulator, validators are essential for achieving consensus and validating transactions through a process known as [Optimistic Democracy](/concept/optimistic-democracy). When you initialize the simulator based on your selected LLM provider(s), you are provided with 5 default validators.
...
2. **Validators:** After the leader proposes a transaction execution, other validators are responsible for reviewing and validating the proposal. They use the [Equivalence Principle](/concept/optimistic-democracy/equivalence-principle) to determine if the leader’s proposal meets the required standards.
...
1. On the left sidebar, click on the **Settings** icon. This will open the settings page.

2. In the settings page, you will see the **Validators** section. This section lists all the validators currently configured in the simulator along with their models and providers.
