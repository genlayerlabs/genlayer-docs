...
## Sending Value (Payable Methods)

To send GEN tokens to a payable contract method, pass `value` in wei (1 GEN = 10^18 wei):
...
The contract must have the method decorated with `@gl.public.write.payable` — otherwise the transaction will fail.

To send value without calling a specific method (triggers `__receive__` on the contract):
...
const hash = await client.writeContract({
  ...
  functionName: '',
  args: [],
  value: BigInt(10) * BigInt(10 ** 18), // 10 GEN
});
```

See [Balances & Value Transfers](/developers/intelligent-contracts/features/balances) for the contract-side documentation.
