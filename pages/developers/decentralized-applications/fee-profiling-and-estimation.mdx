...
The CLI accepts fee estimate options as JSON. Extract the method profile and pass the same fields to `estimate-fees`:

```bash
genlayer estimate-fees \
  --json \
  --fees '{"leaderTimeunitsAllocation":"125","validatorTimeunitsAllocation":"250","executionBudgetPerRound":"786297","totalMessageFees":"0","appealRounds":"1","rotations":["1","1"]}'
```

Then pass the printed transaction fee object to `genlayer write` or `genlayer deploy`:

```bash
genlayer write 0x123456789abcdef create_bet \
  --fees '{"distribution":{"leaderTimeunitsAllocation":"125","validatorTimeunitsAllocation":"250","executionBudgetPerRound":"786297","totalMessageFees":"0","appealRounds":"1","rotations":["1","1"]},"feeValue":"..."}' \
  --args "2024-06-20" "Spain" "Italy" "1"
```
