...
All emitted messages (both internal and external) can have attached `value`: `ContractAt(...).emit(value=100).my_method()`
...
class MultiContractInteraction(gl.Contract):
    ...
    def forward(self) -> None:
        ...
        token.emit_transfer(value=amount)
    ...
    def donate_twice(self) -> bool:
        ...
        if self.balance < amount:
            return False
        token.emit(value=amount*2).donate_twice()
        return True
```
