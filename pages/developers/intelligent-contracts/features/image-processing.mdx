...
class ReceiptVerifier(gl.Contract):
    ...
    def verify_receipt(self, image_data: bytes, expected_amount: str) -> None:
        def leader_fn():
            image = gl.nondet.Image(raw=image_data, pil=None)
...
The `images` parameter accepts a sequence of `bytes` or `gl.nondet.Image` objects.
...
<Callout type="info">
  Image processing requires a vision-capable LLM model (e.g., GPT-4o, Claude Sonnet). Ensure your validator configuration includes a model that supports image inputs.
</Callout>
