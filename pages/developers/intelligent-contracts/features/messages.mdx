...
An internal message is an **asynchronous** call from one Intelligent Contract to another. It stays within the GenVM layer and creates a child transaction that goes through consensus independently. The calling contract does not wait for the message to be processed — execution continues immediately after emitting.
