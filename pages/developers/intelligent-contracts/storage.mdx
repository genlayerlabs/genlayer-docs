...
@storage
class B:
    empty: TreeMap[str, str]

# Creating an instance of B and accessing the empty TreeMap
newMap = B().empty
```
This ensures that `newMap` is an empty `TreeMap[str, str]` that can be modified within the contract.
