...
# Development Tips
Here are some valuable tips on how to develop your intelligent contract, debug it, and test it using the Genlayer Simulator.

## 1. Validators setup
You need to be sure that you have at least one validator. If you don't have any, you can create one in the settings screen by providing:
- Provider
- Model
- Stake (at least 1)
- Config (optional)
...
```
Right now, your contract must include a constructor. Without one, the simulator won't be able to deploy your contract.
...
<br/>You can then call the contract's methods, shown below in the deployment section on the left panel of the simulator's UI.
<br/>
<Image src="/images/run-debug/read-write-methods.png" alt="Node Log" width={400} height={400} />

## 5. Debugging your contract
You can debug your contract by adding print statements to your contract methods. The simulator's Logs section will display these print statements.
...
If your contract methods change the contract's state, you can query that information by adding a read method to your contract (read methods should start with `get_`). This method should return the state you want to query.
<br/>For example:
```python
...
```
These two methods allow you to query the contract's balance state. You can call them by expanding them from the left panel of the simulator's UI and clicking on the "Call Contract" button.