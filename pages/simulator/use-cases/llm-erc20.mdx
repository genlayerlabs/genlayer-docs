...
You can access and use this example project from GitHub: [ERC-20 Token Contract Example](https://github.com/yeagerai/genlayer-simulator/blob/main/examples/contracts/erc20_token.py)
...
This process ensures that every transaction is processed correctly and consistently, maintaining the integrity of the token system. You can view the logs to see detailed information about the contract interaction.

Additionally, you can use the getter functions `get_balances()` to see the balances of all users and `get_balance_of(address)` to check the balance of a specific user, ensuring the contract's logic has been correctly executed. You can view the logs to see detailed information about the contract interaction.