...
For example, consider this `WizzardOfCoin` contract, which is designed to manage a magical coin that should not be given away. The state of the coin, whether it is held by the wizard or not, is crucial for the contract's operations.
...
"""
        ...
        return result_json
```
For this contract we will enter the JSON object that matches the contract's constructor signature. For the WizzardOfCoin contract, we are using the constructor parameter `have_coin`. 
```json
{
    "have_coin": true
}
...
The value will set whether the wizard has the coin at the start. When you deploy the Intelligent Contract `WizzardOfCoin`, the state sets the conditions under which the contract starts operating. 
