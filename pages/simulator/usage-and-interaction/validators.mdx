...
In the GenLayer Simulator, validators are essential for achieving consensus and validating transactions through a process known as Optimistic Democracy. When you initialize the simulator based on your selected LLM provider, you are provided with 5 default validators.
...
2. **Validators:** After the leader proposes a transaction execution, other validators are responsible for reviewing and validating the proposal. They use the Equivalence Principle to determine if the leader’s proposal meets the required standards.
