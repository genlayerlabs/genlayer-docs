...
In this guide, we'll explore the Intelligent Contract syntax using the **Wizard of Coin** example, providing you with the foundational knowledge to create your own Intelligent Contract.
...
## Importing Modules
Before you start coding your Intelligent Contract, make sure to import the necessary modules. These imports provide the functionalities required for your contract’s execution within the GenLayer environment.
...
### Key Imports
- **`IContract`**: Imported from `genvm.base.icontract`, this is the base class for all Intelligent Contracts on GenLayer. It provides your contract with essential properties and behaviors necessary for effective operation within the GenVM environment.

- **`call_llm_with_principle`**: For our example, we are importing this function from `genvm.base.equivalence_principle`. Its simple and direct for making single LLM calls that adhere to the Equivalence Principle, making outcomes consistent and reliable across different validators.

**Other imports from `genvm.base.equivalence_principle`**

Depending on the specific needs of your Intelligent Contract, you can import any of the following functions from `genvm.base.equivalence_principle`:

  - **`EquivalencePrinciple`**: Ideal for contracts involving multiple non-deterministic operations like multiple calls to the LLMs or . This component helps ensure that all actions conform to a standardized validation of outcomes, crucial for maintaining consistency across various validators.

  - **`get_webpage_with_principle`**: Best suited for simpler contracts focused on web data retrieval.

### Conditional Import

- **`json`**: Import this module only if your contract involves handling JSON data, such as parsing inputs or formatting outputs.
