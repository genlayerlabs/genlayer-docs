...
Intelligent Contracts extend beyond traditional smart contract capabilities by interacting with the web and processing natural language (NLP) instructions. This makes it suitable for applications that require dynamic data interaction and complex decision-making processes. These contracts are written in Python and executed within the GenVM (GenLayer's specialized secure virtual environment).

import { Callout } from 'nextra-theme-docs'

<Callout emoji="👾">
  The syntax presented here is a temporary placeholder used in the simulator. For the GenLayer blockchain the final syntax will include specialized keywords that streamline some of the processes currently handled through standard Python code. You can define any number of parameters and include any initialization logic as required by your contract.
</Callout>
...
- `IContract`: This is imported from `genvm.base.icontract` and it is the base class for all Intelligent Contracts on GenLayer. It equips your contract with essential properties and behaviors required for effective functioning within the GenVM environment.

When importing from `genvm.base.equivalence_principle`, you can choose between `EquivalencePrinciple`, `call_llm_with_principle`, or `get_webpage_with_principle` based on the complexity of your contract:

- `EquivalencePrinciple`: Use this if your contract is involves multiple non-deterministic operations like accessing data from the Web or querying an LLM to maintain consistency across different validator nodes. 

- `call_llm_with_principle` or `get_webpage_with_principle`:  These are helper functions provided under the EquivalencePrinciple framework. They are suitable for simpler contracts that focus on a single type of non-deterministic operation:

   - Use `call_llm_with_principle` if your contract primarily interacts with LLMs
   - Use`get_webpage_with_principle` if it primarily retrieves web data. 
   
These functions are designed to handle single types of non-deterministic operations
...
<Callout type="info" emoji="ℹ️">
  In the constructor method  `__init__`, you can execute any Python code and add as many parameters as needed. The names `initial_state` and `state_variable` are placeholders—customize them to fit your intelligent contract’s context.
</Callout>
...
Once a result has been validated and finalized within the Equivalence Principle block, it can be parsed for easier data handling. In our [example prompt](http://localhost:3000/simulator/intelligent-contract-syntax#crafting-prompts-for-llm-and-web-browsing-interactions), we requested our validator to produce the output in a JSON format and for that we are using the `json.loads` to parse it. 
