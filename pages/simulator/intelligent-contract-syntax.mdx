...
For example, consider an intelligent contract designed to summarize a news article. The Equivalence Principle specifies that _the summary should capture all key points accurately_. The leader executes this task and the validators do not generate their own summaries; instead, they evaluate the leader’s summary against the criteria set by the Equivalence Principle to determine accuracy and completeness.
