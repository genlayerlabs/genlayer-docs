...
`get_webpage_with_principle` It is useful when the data fetched does not need to be further extracted, e.g., if you're getting the data from an API. If you're getting data from a normal webpage, you'll usually need to have the next step as an LLM call to extract the data you want.
