...
`get_webpage_with_principle` is useful when the data fetched is already in a structured and usable format, such as data from an API. In these cases, no further extraction or processing is needed. However, if you're retrieving data from a standard webpage, the data is often unstructured and will usually require an additional step, such as an LLM call, to extract and format the specific information you need.
