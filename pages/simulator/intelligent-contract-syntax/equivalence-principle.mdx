...
The `EquivalencePrinciple` is used for contracts that involve complex operations, such as:
- **Multiple LLM Calls**: Asking different language models for their input or analysis on a topic.
- **Data Fetching and Processing**: Gathering information from various sources, then organizing or transforming that data.

This function gives you detailed control over how the outputs are validated. Depending on how you want the validators to work, you can choose either the **comparative** or **non-comparative** Equivalence principle options.
...
The `call_llm_with_principle` function is an abstraction from the `EquivalencePrinciple`, used for contracts that require a single LLM call. It simplifies the validation process, allowing you to focus on the core logic of your contract. You can only use the **non-comparative** equivalence principle option with this function.
...
The `get_webpage_with_principle` function is an abstraction from the `EquivalencePrinciple`, used to retrieve data from webpages. It operates without the comparative option, as ensuring the accuracy of data fetched from web sources requires direct verification by validators or an LLM to extract and format the specific information you need.
