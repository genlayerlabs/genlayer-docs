...
The `call_llm_with_principle` is an abstraction from the `EquivalencePrinciple` function, used for contracts that require a single LLM call. It simplifies the validation process, allowing you to focus on the core logic of your contract. For this principle, you can use either the **Comparative** or **Non-Comparative** Equivalence principle options.
...
The `get_webpage_with_principle` is an abstraction from the `EquivalencePrinciple` function, used to retrieve data from webpages. For this principle, only the **Comparative** Equivalence principle option is used as data fetched from web sources requires verification by validators i.e Validators will perform similar tasks and compare their results to the leader's result to ensure it falls within this margin.
